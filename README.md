...
**배포 승인 / 미승인 / 보류** 결론과 조치 방법을 한국어 보고서로 줍니다.
코드는 PC 밖으로 나가지 않고, 판정 근거(엔진·룰셋·위협 정보 수집 시각)가 보고서에 남습니다.

![License](https://img.shields.io/badge/License-PolyForm%20Noncommercial%201.0.0-blue.svg)
![Python](https://img.shields.io/badge/Python-3.11+-green.svg)
![Tests](https://img.shields.io/badge/tests-2083_passed-success.svg)
...
1. **검사** — 폴더를 주면 소스 코드(Python·JavaScript·TypeScript), 사용 중인 패키지(PyPI·npm), 폴더에 직접 넣어둔 라이브러리 파일(`static/*.min.js`)까지 한 번에 봅니다. 코드는 외부로 전송되지 않습니다.
2. **판정** — 발견을 심각도(치명·높음·보통)와 조치(차단·경고·허용)로 나누고, 코드와 패키지를 **함께** 보고 결론 하나를 냅니다: **배포 승인 가능 / 미승인(차단) / 보류(확인 필요)**. 확인하지 못한 것은 "안전"이 아니라 "판정 불가"로 적습니다.
...
| **망분리(폐쇄망) 지원** | `GVSKB_MODE=offline` 설정으로 외부 통신을 완전히 차단하고 동작합니다. 위협 정보(취약점·악성 패키지·실제 악용 목록·국내 보안공지)는 매일 자동 생성되는 반입 번들 하나로 옮기며, 파일별 sha256으로 변조를 검증합니다. |
| **판정의 한계를 표시** | "판정하지 못함"을 "안전"으로 바꿔 말하지 않습니다. 검사한 파일이 0개면 경로를 확인하라고 말하고, 캐시가 오래되면 판정을 보류로 낮추고, 검사 엔진이 빠졌으면 어떤 엔진이 빠졌는지 결과에 남기고, 탐지 못 하는 항목은 성능 표에 공개합니다. |
| **판정을 재현할 수 있음** | 보고서마다 엔진 버전·룰셋 지문·위협 정보 수집 시각이 기록되고, `gvskb status` 로 설치본이 어느 커밋인지 확인됩니다. 어제 통과한 코드가 오늘 차단되면 코드 때문인지 룰 때문인지 구분할 수 있습니다. |
...
```text
── 요약층 (공무원) ─────────────────────────────
| 대상 | C:\…\민원처리앱 |  판정 기준 | 엔진 0.4.0 · 룰셋 2026.09.18a |
...
gvskb status      # 설치본의 버전·커밋·룰셋 — 여러 PC 에 같은 판정 기준이 깔렸는지 대조할 때
```
...
각 발견의 심각도는 `치명 → 높음 → 보통`, 조치 등급은 `필수 조치(block) → 경고(warn) → 허용(allow)` 으로 표시됩니다. 결론 박스에는 **판정 기준표**(무엇이 차단이고 무엇이 조건부 승인인지)와 **해소 방안**(무엇을 하면 차단이 풀리는지)이 함께 실립니다. 요약에는 판정 근거의 확신 수준(확인됨 · 유력함 · 패턴 일치만)이 구분되고, 차단 건수도 "확인 / 유력 / 패턴 후보"로 나뉘어 값의 출처를 직접 확인해야 하는 항목이 무엇인지 알 수 있습니다.

**XSS(innerHTML 등)는 값의 출처로 등급이 갈립니다.** 상수 문자열은 발견에서 빠지고, 프로젝트가 만든 정화 함수(`esc()` 같은 이름이어도 본문이 실제로 이스케이프하면 인정)를 거친 값은 경고, 출처를 추적하지 못한 값은 **경고(정밀 검토)** 로 남으며, URL·요청·입력값·서버 응답이 정화 없이 들어가는 것만 **필수 조치**입니다. 이름만 정화인 함수(`sanitizeMaybe(h){return h.trim()}`)는 인정하지 않고, 분석 엔진이 실패하면 완화 없이 차단이 남습니다.
...
- **실행 소스 파일이 크기 상한(기본 8MB)을 넘어 검사되지 않으면** 그 사실이 `coverage.oversized_source_files` 에 남고 승인 판정이 나오지 않습니다(`--max-file-bytes` 로 상한 조정). 빠진 파일은 "이상 없음"이 아니라 "판정 밖"입니다.
...
| 출처 | 반영 |
|---|---|
| **KISA 시큐어코딩 가이드 (2023 개정)** | Python 45항목 · JavaScript 36항목 |
...
검사에 쓰이는 승인 룰은 **223개**입니다 — 탐지 룰 103개(검사에서 발견을 만드는 룰) + 참조 룰 120개(발견의 근거·출처로 인용되는 지식 룰). 그 밖에 CISA KEV에서 매일 자동 생성되는 초안 카드가 있지만 사람이 승인하기 전에는 판정에 쓰이지 않습니다. 모든 룰은 Markdown 파일로 정의되어 누구나 읽고 검토할 수 있습니다.
...
- **인터넷 PC**: 별도 설정 없이 자동입니다. MCP 서버가 뜰 때 캐시 신선도를 확인하고 낡았으면 하루 1회 번들을 받아옵니다(`GVSKB_AUTO_UPDATE=off` 로 끌 수 있음). 터미널만 쓴다면 `gvskb intel-sync` 한 번이면 됩니다.
- **망분리 PC**: [아래 절차](#망분리폐쇄망-환경에서-쓰기)대로 번들을 반입합니다. 관리자가 공유 폴더(`GVSKB_INTEL_DIR`)에 번들을 놓아두면 각 PC가 자동 반영합니다.
- **서버·포털에서 쓸 때**: 검사를 실행하는 계정의 캐시가 쓰입니다. 서비스 계정이 따로 있으면 `GVSKB_CACHE_DIR` 로 캐시 폴더를 지정하세요. 갱신 잡의 소스별 결과(항목 수·수집 시각·커버리지)는 릴리스 노트에 매일 기록됩니다.
...
| 값의 출처 추적 | JS 는 파서 없이 줄 단위로 변수를 따라갑니다. 함수 경계를 넘거나 다른 모듈에서 오는 값은 "출처 미상"으로 남아 사람이 확인해야 합니다 |
...
2. 검사 중 PC가 직접 부르는 외부 주소는 **OSV(취약점 조회)와 PyPI·npm 레지스트리(패키지 실재·발행일)** 뿐이고, 보내는 것은 패키지명·버전 같은 공개 식별자입니다. NVD·KNVD·CISA KEV·EPSS는 검사 중 접속하지 않습니다 — GitHub Actions가 매일 받아 번들로 제공하고 PC는 그 캐시만 읽습니다.
3. `GVSKB_MODE=offline` 설정 시 외부 통신을 완전히 차단하고, 반입한 캐시와 로컬 룰만으로 동작합니다. 이때 자동 갱신은 공유 폴더(`GVSKB_INTEL_DIR`)만 확인하고 인터넷 주소는 시도하지 않습니다.
4. 인터넷이 제한된 서버(GitHub는 되고 OSV는 안 되는 경우)는 offline 모드로 두고, 예약 작업이 GitHub 릴리스에서 번들을 받아 `GVSKB_INTEL_DIR` 폴더에 놓게 하면 됩니다. 서버가 열어야 할 외부 주소는 github.com 하나입니다.
...
캐시 위치는 `%USERPROFILE%\.gvskb\cache` 입니다(환경변수 `GVSKB_CACHE_DIR` 로 변경 가능). 반입 결과는 `gvskb update-intel --all --from-cache` 로 소스별 항목 수·수집 시각·신선도를 확인할 수 있습니다.
...
- 보고서에 **어느 날짜 캐시 기준의 판정인지**(피드별 수집 시각)가 자동 표기됩니다. KEV 등재 취약점에는 EPSS 악용확률·CVSS 점수가, CVE가 정확히 일치하는 취약점에는 KISA KNVD 국내 보안공지 링크가 함께 실려 보안팀이 우선순위를 정하고 국내 공지를 바로 확인할 수 있습니다.
...
보고서 머리말의 「판정 기준」 줄에 `엔진 x.y.z · 룰셋 YYYY.MM.N` 이 항상 기록됩니다 — 판정을 재현하려면 둘 다 같아야 합니다. 여러 PC·서버에 깔린 설치본이 같은 커밋인지는 `gvskb status --json` 의 `install_identity.commit_id` 로 대조합니다(wheel 설치본도 빌드 시 커밋이 기록됩니다).
