...
![독립 벤치마크](https://img.shields.io/badge/독립_벤치마크-recall_100%25_(37%2F37)-success.svg)
...
- [결과 읽는 법](#결과-읽는-법)
- [무엇을 탐지하나](#무엇을-탐지하나)
- [성능 — 측정 결과와 한계](#성능--측정-결과와-한계)
- [오탐(false positive)을 어떻게 다루나](#오탐false-positive을-어떻게-다루나)
...
룰은 총 **326개**이며 두 종류로 나뉩니다 — **탐지 룰 99개**(검사에서 실제 발견을 만드는 룰)와 **참조 룰 227개**(근거·지식 카탈로그로 인용되는 룰). 총계를 탐지 능력으로 읽지 않도록 도구 스스로 나눠서 보고합니다. 모든 룰은 Markdown 파일 한 장으로 정의되어 누구나 읽고 검토·확장할 수 있습니다.
...
- 최근 90일 내 등재된 CISA KEV 취약점은 초안 지식 카드로 자동 수록됩니다. 단, **검사 판정에 반영되는 것은 사람이 `status: approved` 로 승격한 룰뿐**입니다 — 자동화는 수집까지, 판정 반영은 사람이 결정합니다.

---

## 성능 — 측정 결과와 한계

서로 다른 두 가지를 분리해 측정합니다: ① 실전형 취약점을 잡는가, ② 일부러 숨긴 우회 변형까지 잡는가. 하나로 합치면 한계를 공개할수록 숫자가 나빠져, 한계를 숨길 유인이 생기기 때문입니다.

| 측정 | 결과 | 방식 |
|---|---|---|
| **독립 벤치마크 탐지율** | **recall 100% (37/37) · 오탐 0** | `eval_corpus/` 시드 37개(Flask·Express·정적 페이지·LLM 챗봇·데이터 파이프라인) + 음성 대조군 6파일, 결정론적 실행 |
| 경계 프로브(의도적 우회 변형) | **4/9 탐지** | 별칭 import·간접 eval·난독화 JS 등 — 한계 측정이 목적이며 미탐을 그대로 공개 |
| 알려진 공백(known gap) | **15건 공개** | 외부 보안 지침이 요구하지만 현재 탐지하지 못한다고 확인된 항목 — recall 에 포함하지 않고 별도 목록으로 유지 |
| 자체검증(룰 내장 예제) | P/R/F1 100% | 룰이 자기 예제를 맞히는지 측정 — 외부 코드 성능 지표가 아님 |
| 테스트 | **1,589개 통과** | 유닛·통합·룰 회귀 |

재현 방법: `GVSKB_MODE=offline PYTHONPATH=src python scripts/run_benchmark.py` (결과는 `eval_corpus/results/`에 저장)

> **측정 원칙** — "자체검증 100%"는 룰이 자기 예제를 맞힌다는 뜻이지 외부 코드에 대한 성능이 아닙니다. 그래서 독립 코퍼스로 따로 측정하고, 탐지하지 못하는 항목(경계 프로브 5건 미탐, known gap 15건)을 숫자와 함께 공개합니다. 이 도구는 **보안 전문 검토를 대체하지 않는 1차 점검 도구**로 설계되었습니다.

---

## 오탐(false positive)을 어떻게 다루나

이 도구는 놓침(미탐)보다 한 번 더 알리는 쪽을 택한 **재현율 우선** 설계입니다. 따라서 맥락상 안전한 코드가 표시될 수 있으며, 발견 사항은 증거 코드와 함께 사람이 확인하는 것을 전제로 합니다. 오탐을 줄이기 위해:

- **빌드 산출물 자동 제외** — 압축·번들·캐시(`dist/`·`public/assets/`·해시 파일명 `app-3f9a2c1b.js`)는 원본 소스가 아니므로 검사에서 제외하고, 보고서에 제외 사유와 건수를 표기합니다.
- **다만 `*.min.js` 는 제외하지 않습니다** — 폴더에 받아둔 외부 라이브러리는 프로젝트가 실제로 실행하는 코드입니다. 소스 룰 검사에서만 빼고(압축 코드는 오탐만 생성) 라이브러리·버전을 식별해 알려진 취약점을 따로 검사합니다. 버전을 확신할 수 없으면 추측하지 않고 '판정 불가'로 남깁니다.
- **실측 기반 룰 정밀화** — 실제 프로젝트에서 오탐이 확인된 룰(SQL 삽입·XSS·전화번호 등)은 문맥·예외 패턴으로 좁힙니다. 예: `DOMPurify.sanitize()` 로 정화한 코드, 파라미터 바인딩(`?`) 쿼리, 예시·마스킹 전화번호는 표시하지 않습니다.
- **정밀화 ≠ 민감도 낮추기** — 모든 정밀화는 룰의 오탐 회귀 예제와 독립 벤치마크로 미탐이 늘지 않았음을 확인한 뒤에만 반영합니다. 룰을 끄거나 느슨하게 만드는 방식은 쓰지 않습니다.
- **오탐을 숨기지 않고 기록으로 관리** — 오탐이거나 기관이 위험을 수용한 항목은 [승인된 예외](#ci자동화--게이트--sbom--예외-관리)로 등록합니다. 승인자·사유·만료일이 있어야 유효하고, 보고서와 감사로그에 남습니다.
...
**처음 도입한다면 `--fail-on dependency` 부터 시작하세요.** 이유: 의존성 판정("이 버전에 이 CVE가 있다")은 사실 조회라 오탐이 거의 없지만, 소스 코드 판정은 위험한 형태를 찾는 추론이라 맥락에 따라 오탐이 있습니다. 소스 오탐 때문에 팀이 게이트를 끄면 오탐 없는 의존성 차단까지 함께 꺼집니다. 신뢰가 쌓인 뒤 `block` 으로 올리세요. 결과 JSON·MCP 응답에는 `blocked_source` · `blocked_dependency` 가 분리되어 있어 직접 정책을 구성할 수도 있습니다.
