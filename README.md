...
![Tests](https://img.shields.io/badge/tests-358_passed-success.svg)
...
보고서는 **1페이지 요약**(결론 배너 → *다음 3단계 행동 안내* → 핵심 숫자 → 위험 유형·파일별 요약 → 가장 먼저 할 일)으로 시작하고, 그 아래로 **파일별 상세 · 수정 프롬프트 · 🌐 외부 연결 인벤토리**가 이어집니다. 긴 섹션은 기본 접혀 있어 한눈에 들어오고, 인쇄(→PDF) 시 자동으로 펼쳐집니다.
...
이 도구는 *놓침(미탐)보다 한 번 더 알림*을 택한 **재현율 우선** 설계입니다. 따라서 실제 코드에서는 맥락상 안전한데도 표시되는 항목이 있을 수 있어, **발견 사항은 증거 코드와 함께 사람이 맥락을 확인**하는 것을 전제로 합니다(리포트의 "다음 3단계" 안내 참고). 오탐을 줄이기 위해:
