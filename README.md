...
![Tests](https://img.shields.io/badge/tests-1038_passed-success.svg)
...
> 배포 판정 · 배포 불가 — 차단 2건과 차단 기준에 걸린 패키지를 해소하거나
> 보안담당자 승인이 필요합니다.  의존성: 취약·악성 패키지 3건 ·
> 판정 불가 1건('안전'이 아닙니다).      ← 코드와 패키지를 함께 보고 판정

## 요약
- 발견된 위험 7건 · 차단 2건 · 최고 심각도: 치명
- 취약 패키지 3건 · 승인된 예외 1건 (요약에서 제외)
...
## 의존성(패키지) 취약점 검사
### 컴포넌트 (고유 3종)         ← 같은 패키지가 여러 곳에 있어도 한 줄로
| 패키지  | 버전   | 판정          | 출처                      |
| pillow  | 12.2.0 | ⚠ 취약점 26건 | requirements.txt · 설치본 |
| xlsx    | 0.18.5 | ⚠ 취약점 2건  | 벤더 번들                 |
| qrcode  | —      | 판정 불가     | 벤더 번들                 |
```

> 위 표에서 `pillow` 는 `requirements.txt` 와 설치본 양쪽에 있지만 **한 줄로 셉니다** —
> 고쳐야 할 것은 패키지 하나이기 때문입니다. `xlsx` 는 `package.json` 없이
> `static/` 에 직접 넣어둔 파일에서 찾아낸 것입니다.
...
기관 내 설명·교육·보안성 검토용 자료(공무원용 안내, 전문가용 기술 설명서, 인텔 운영
구조, 운영 계획)는 **도입 기관에 직접 제공**합니다. 필요하시면
[Discussions](https://github.com/Lex6won/vibecode-checker/discussions)로 문의해 주세요.

바로 쓸 수 있는 문서는 아래 둘입니다.

| 자료 | 대상 | 내용 |
|---|---|---|
| [30초 시작 가이드](docs/quickstart_ko.md) | 처음 쓰는 분 | 설치·첫 검사·결과 읽는 법 한 장 요약 |
| [Windows 한글 설정](docs/windows_utf8.md) | Windows 사용자 | 콘솔 한글 깨짐 1회 설정 |
...
| 폴더에 받아둔 외부 라이브러리 | `static/xlsx.full.min.js` 처럼 **직접 넣어둔 자바스크립트 파일** — 어떤 라이브러리 몇 버전인지 찾아 알려진 취약점을 대조 (`package.json` 이 없어도 검사합니다) |
...
결론은 **내 코드와 사용 중인 패키지를 함께** 보고 정해집니다.

| 결론 박스 | 뜻 | 언제 뜨나 | 무엇을 하나요 |
|---|---|---|---|
| **배포 승인 가능** | 심각한 위험 미발견 | 코드·패키지 **둘 다** 이상 없음 | 단, '검토 범위·한계'는 확인 |
| **배포 미승인 (차단)** | 그대로 배포하면 위험 | 차단 발견 **또는** 차단 기준에 걸린 패키지 | 먼저 고치거나 보안담당자 승인 |
| **배포 보류 (확인 필요)** | 주의 항목 있음 | 경고 발견 **또는** 취약·판정 불가 패키지 | 확인·수정 후 배포 |

> **코드가 깨끗해도 초록불이 아닐 수 있습니다.** 쓰고 있는 패키지에 알려진 취약점이
> 있거나, 인터넷이 막혀 패키지를 확인하지 못했으면(= *판정 불가*) 초록불을 주지
> 않습니다. **판정 불가는 '안전'이 아니라 '아직 모른다'** 는 뜻입니다.
...
트리거 단어: **보안 · 점검 · 체크 · 검토 · 검사 · 스캔 · "안전한지"**. 잘 안 불러오면 *"vibecode-checker로 점검해줘"* 라고 덧붙이세요.
...
> 핵심 룰 **219개** (자동 탐지 99 + 지식·참조 120) + 매일 갱신되는 **인텔 초안 카드**(가변 — 최근 90일 KEV, 자동 수명주기). 모든 룰은 Markdown 한 장으로 정의돼 누구나 읽고 검토·확장할 수 있습니다.
...
서로 다른 두 가지를 **분리해서** 측정합니다 — ① 실전형 취약점을 잡는가(품질 보증),
② 일부러 숨긴 우회 변형까지 잡는가(도구의 한계). 한 지표로 합치면 한계를 공개할수록
숫자가 나빠져 정직성이 훼손되기 때문입니다.

```text
■ 측정 1 — 대표 시드 30개 (실전형 취약점 · 품질 보증 지표)
개인정보(PII)      ████████████████████  100%  (5/5)
시크릿·키          ████████████████████  100%  (4/4)
주입(SQL·명령)     ████████████████████  100%  (5/5)
코드 실행          ████████████████████  100%  (3/3)
...
───────────────────────────────────────────────────
종합 recall        ████████████████████  100%  (30/30) · 오탐 0

■ 측정 2 — 경계 프로브 9개 (의도적 우회 변형 · 한계 측정, 미탐이 정상)
탐지               █████████░░░░░░░░░░░   44%  (4/9)
남은 미탐: 별칭 import · 간접 eval · 난독화 JS · Java SQL 조립
```
...
- **빌드 산출물 자동 제외** — 압축·번들·캐시(`dist/`·`public/assets/`·`.puppeteer-cache`·`.tmp`·해시 파일명 `app-3f9a2c1b.js`)는 원본 소스가 아니므로 검사에서 빼고, 리포트에 *"빌드 산출물 N건 제외 — 원본 아님"*으로 표기합니다.
- **다만 `*.min.js`는 빼지 않습니다** — 폴더에 받아둔 외부 라이브러리(`static/xlsx.full.min.js` 등)는 빌드 결과물이 아니라 **프로젝트가 실제로 실행하는 남의 코드**입니다. 소스 룰 검사에서만 빼고(압축된 코드라 오탐만 납니다) **어떤 라이브러리·몇 버전인지 찾아 알려진 취약점을 따로 검사**합니다. 버전을 확신할 수 없으면 추측하지 않고 *'판정 불가'* 로 남깁니다.
